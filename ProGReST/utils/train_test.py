...
def _train_test(model, criterion, dataloader, optimizer, device, pt=0, cl=0, prob=0, ld=0, mse=0, task_type='cls', mean=0, std=1, kmeans=None, scheduler=None, epoch=0):
    stats = {'pred': [], 'correct': []}
    ...
    #        total_stats['roc'] = roc_auc_score(stats['correct'], np.array(stats['pred']),multi_class='ovo')
        ...
        # print(stats['pred'][:5])
        # print(stats['correct'][:5])
        stats['pred'] = np.stack(stats['pred'])
        stats['correct'] = (torch.stack(stats['correct'])).detach().numpy()
...
def train(model, criterion, dataloader, optimizer, device, pt=0, cl=0, prob=0, ld=0, mse=0, task_type='cls', mean=0, std=1, kmeans=None, scheduler=None, epoch=0):
    model.train()
    return _train_test(model, criterion, dataloader, optimizer, device, pt, cl, prob, ld, mse, task_type, mean, std, kmeans, scheduler, epoch)

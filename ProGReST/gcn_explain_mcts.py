...
def explain_reg_mcts(args, features, generate_base_exp=False):
    ...
    print(device)
    protoTree.load_state_dict(torch.load(f'/home/ubuntu/ProGReST/saves/{args.dataset_name}_{model_name}_proj.pt', map_location=device))
    protoTree.eval()
    ...
    if generate_base_exp:
        with torch.no_grad():
            ...
            for idx in range(protoTree.num_prototypes):
                ...
                mol_idxs = list(np.random.randint(len(train_dataset),size=10)) + [prototype_idxs[idx].item()]
                best_p = 0
                best_idx = 0
                best_coalition = []
                best_proto = None
